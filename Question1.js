...
function ReverseAndGetPrimeFactor(x) {
    ...
    let reverseNumber = reverseInteger(x);
    let primFactor = [];
    for (let i = 2; i <= reverseNumber; i++) {
        if (isPrime(i) && reverseNumber % i === 0) {
            primFactor.push(i);
            reverseNumber /= i;
            i--;
        }
    }
    if (primFactor.length > 0) {
        return `Yes - [${primFactor.join(',')}]`
    } else {
        return "no";
    }
}
...
console.log(ReverseAndGetPrimeFactor(123));
